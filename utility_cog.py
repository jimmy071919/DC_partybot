...
class Utility(commands.Cog):
    ...
    async def random_pick(self, interaction: discord.Interaction):
        ...
        if len(members) < 2:
            await interaction.response.send_message(
                "語音頻道中至少需要2個人才能抽籤！", ephemeral=True
            )
            return
    ...
    async def poll(self, interaction: discord.Interaction, question: str, options: str):
        ...
        for i, option in enumerate(options_list):
            embed.add_field(
                name=f"{number_emojis[i]} {option}", value="\u200b", inline=False
            )
    ...
    async def userinfo(
        ...
        embed = discord.Embed(title=f"{member.name}的用戶資訊", color=member.color)
        ...
        embed.add_field(
            name="加入時間", value=member.joined_at.strftime("%Y-%m-%d %H:%M:%S")
        )
        embed.add_field(
            name="帳號建立時間", value=member.created_at.strftime("%Y-%m-%d %H:%M:%S")
        )

        roles = [role.mention for role in member.roles[1:]]  # 排除@everyone
        embed.add_field(
            name=f"身分組 [{len(roles)}]",
            value=" ".join(roles) if roles else "無",
            inline=False,
        )
