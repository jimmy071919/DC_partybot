...
class Utils(commands.Cog):
    ...
    async def check_reminders(self):
        ...
        try:
            ...
            if current_time in self.reminders:
                self.logger.info(
                    f"發現 {len(self.reminders[current_time])} 個待處理提醒"
                )
    ...
    async def clear(self, interaction: discord.Interaction, amount: int = 5):
        ...
        if not interaction.user.guild_permissions.manage_messages:
            await interaction.response.send_message(
                "你沒有權限執行此指令！", ephemeral=True
            )
            return

        if amount < 1 or amount > 100:
            await interaction.response.send_message(
                "請輸入 1-100 之間的數字！", ephemeral=True
            )
            return
    ...
    async def userinfo(
        ...
        embed = discord.Embed(
            title=f"👤 {member.display_name} 的資訊", color=member.color
        )

        embed.set_thumbnail(url=member.display_avatar.url)
        embed.add_field(name="用戶 ID", value=member.id, inline=True)
        embed.add_field(
            name="加入時間", value=member.joined_at.strftime("%Y/%m/%d"), inline=True
        )
        embed.add_field(
            name="帳號建立時間",
        ...
        embed.add_field(
            name="身分組",
            value=" ".join([role.mention for role in member.roles[1:]]) or "無",
            inline=False,
        )
    ...
    @app_commands.describe(minutes="幾分鐘後提醒（1-1440）", message="提醒內容")
    async def set_reminder(
        self, interaction: discord.Interaction, minutes: int, message: str
    ):
        """設定提醒"""
        # 驗證參數
        if minutes <= 0:
            await interaction.response.send_message(
                "⚠️ 請輸入大於 0 的分鐘數！", ephemeral=True
            )
            return

        if minutes > 1440:  # 24小時 = 1440分鐘
            await interaction.response.send_message(
                "⚠️ 提醒時間不能超過 24 小時（1440 分鐘）！", ephemeral=True
            )
            return

        if not message or len(message) > 200:
            await interaction.response.send_message(
                "⚠️ 提醒內容不能為空且不能超過 200 個字元！", ephemeral=True
            )
            return
